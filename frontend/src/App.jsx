...
function AdminPage({ fetchAllData, products, categories, gallery, settings, pages }) {
  ...
  const uploadFileToServer = async (file) => {
    ...
    try { const res = await fetch('http://127.0.0.1:8000/api/upload/', { method: 'POST', body: formData }); return (await res.json()).url; } 
  ...
  const handleGalleryUpload = async (e) => {
    ...
    if (url) {
       await fetch('http://127.0.0.1:8000/api/gallery/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image_url: url }) });
       fetchAllData(); alert("Photo added to Slider!");
    }
  };
  const handleDeleteGallery = async (id) => { await fetch(`http://127.0.0.1:8000/api/gallery/${id}`, { method: 'DELETE' }); fetchAllData(); };

  const handleAddCategory = async (e) => { e.preventDefault(); await fetch('http://127.0.0.1:8000/api/categories/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: catName }) }); setCatName(''); fetchAllData(); };
  const handleDeleteCategory = async (id) => { await fetch(`http://127.0.0.1:8000/api/categories/${id}`, { method: 'DELETE' }); fetchAllData(); };

  const handleLogin = (e) => { e.preventDefault(); if (passwordInput === 'MINERVA') setIsAuthenticated(true); else { alert('DENIED.'); setPasswordInput(''); } };
  const handleAddProduct = async (e) => { e.preventDefault(); await fetch('http://127.0.0.1:8000/api/products/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(productData) }); setProductData({ ...productData, name: '', price: '', tag: '', image_url: ''}); fetchAllData(); alert("Asset added!"); };
  const handleDeleteProduct = async (id) => { if(window.confirm("Delete permanently?")) { await fetch(`http://127.0.0.1:8000/api/products/${id}`, { method: 'DELETE' }); fetchAllData(); } };
  const toggleVisibility = async (product) => { await fetch(`http://127.0.0.1:8000/api/products/${product.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({...product, visibility: !product.visibility}) }); fetchAllData(); };
  const handleCreatePage = async (e) => { e.preventDefault(); await fetch('http://127.0.0.1:8000/api/pages/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...pageData, slug: pageData.slug.toLowerCase().replace(/ /g, '-') }) }); setPageData({ slug: '', title: '', banner_url: '', content: '' }); fetchAllData(); alert(`Page Created!`); };
  const handleDeletePage = async (id) => { if(window.confirm("Delete Page?")) { await fetch(`http://127.0.0.1:8000/api/pages/${id}`, { method: 'DELETE' }); fetchAllData(); } };
  const handleUpdateSettings = async (e) => { e.preventDefault(); await fetch('http://127.0.0.1:8000/api/settings/', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(cmsData) }); fetchAllData(); alert("Website content saved!"); };
...
export default function App() {
  ...
  const fetchAllData = async () => {
    try {
      const pRes = await fetch('http://127.0.0.1:8000/api/products/'); if (pRes.ok) setProducts(await pRes.json());
      const cRes = await fetch('http://127.0.0.1:8000/api/categories/'); if (cRes.ok) setCategories(await cRes.json());
      const gRes = await fetch('http://127.0.0.1:8000/api/gallery/'); if (gRes.ok) setGallery(await gRes.json());
      const pgRes = await fetch('http://127.0.0.1:8000/api/pages/'); if (pgRes.ok) setPages(await pgRes.json());
      const sRes = await fetch('http://127.0.0.1:8000/api/settings/'); if (sRes.ok) setSettings(await sRes.json());
